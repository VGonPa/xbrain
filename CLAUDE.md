...
- **The chunker's parameters are MEASURED now, and the overlap axis was decided by a retriever
  ...
  **0.8179**. *(Round 07, U-6, made this the whole story: with the depth counted in OWNERS the
  sweep's `recall@10` reads 0.8264 for BOTH 800/0 and 1200/150 at every overlap, cell for cell
  on every stratum — the 0.8119-vs-0.8027 advantage and its «+2.1 pp `enterrado`» were the
  chunk-depth artefact, and are retracted; the winner is unchanged and holds by MRR and
  `recall@1`. `docs/knowledge-index.md` carries the regenerated table.)* **Overlap applies only to `video_transcript` and exists so a sentence spanning a
