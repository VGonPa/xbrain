...
- **The persisted index (`data/index/`) is DERIVED, and two of its failure modes are silent.**
  ...
  **0.57 s** (re-measured 2026-09-02 at load 9 — it was 0.26 s until `update` began paying
  `quick_check` in round 06 and the column check in round 07, and the table stood two rounds
  after the cost moved: F7-4), the database **52.4 MB**, **22,286 chunks**, `search` p50 **27.7 ms** at `--limit 10` (35.6 ms
  ...
  `q:<offset>` cursor (A-2), and `get` re-emits the surfaces with the producers the store records
  (A-4; since round 08 an ASR/VLM surface carries NONE — see F7-7 below). §8.7 is measured: a typical `enrich` run re-enriches 31 items (median of 10
  real runs, 2026-05 → 2026-08, range 4–725 — `docs/knowledge-index.md` said 4–333 for the same
  10 runs, the range of a 9-run population whose median is 22; corrected in round 04, H4) and
  leaves 345 chunks stale (1.55 % of 22,286); the largest, a
  1,098-item backfill, 8,503. **Round 03 (2026-09-01, gate Fable) closed two more ways the index could answer OPEN:** `--origin` never reached the PROFILE plane (`search "Forecasting" --origin asr` returned 8 items, 6 with no ASR surface — a profile has no origin, so under an origin filter that plane now contributes nothing, and the totality test covers the eight filters on BOTH planes, G-1); and `index update` — even `--dry-run` — over a deleted `knowledge.db` CREATED an empty base under the standing manifest, after which `search` answered «Sin resultados» with exit 0 while `status` said incomplete (G-2). Now only `build` may create the file, `update` refuses before touching the disk, and every `search` runs the same five-`COUNT(*)` consistency check as `update`/`status` (0.04 ms). The item fingerprint hashes the exact `surfaces` ROW the writer inserts, so a quoted author repaired without touching the body is no longer `0 cambiados` (G-5; a one-time full rewrite on the first `update` after upgrading), a `DatabaseError` past page 1 raised AT QUERY TIME is the rebuild advice instead of a 68-line traceback (G-4 — this line said «on all three commands» until round 06 and that was FALSE for the maintenance reads: see D-1 below), and `xbrain eval` is declared to score the CHUNK plane, not the ranking `xbrain search` serves (top-10 differs in 17 of 23 cases, G-6 — `docs/knowledge-index.md`). **Round 04 (2026-09-02, blind Codex gate over a HEAD another gate had passed):** `topics` stores each topic's members and `stale` bit, both functions of the items' assignments, and `update` decided the whole topic plane from the vocabulary/page fingerprints alone — an `enrich` that moved an item rewrote `item_topics` and left `topics` holding the old members while the manifest and `status` called the index healthy (H1, rule 6 with the instrument lying on top). The topic row is now ONE projection shared by the writer and a comparator (`topic_row`, the G-5 pattern), `update` rewrites only the rows the store no longer agrees with (`topics_refreshed`), and `status` — which now takes `vocab.yaml` and `topics.json` like `build`/`update` — reads them back and reports `topics_changed`, so an index whose item fingerprints all match and whose topic plane is behind is declared. The human continuation of `get` repeats `--surface` and `--query` (H2): a cursor is an offset into the sequence those two define, and the printed `xbrain get ID --cursor C` resumed inside the default selection with an empty page, or was refused — a CLI test now follows the printed line literally on both routes. And the attribution rule this file says was paid for in blood came back on the CHUNK branch of `render_get` (H3): a quoted post paged or prioritised by `--query` arrives as a chunk, whose header printed type, offsets and origin only under the bundle header naming the POSTER — the JSON was right and the human view attributed `@othervoice`'s words to `@vgonpa`. One function now decides for the search match, the surface header and the chunk header: the text's own author is named whenever it differs from the item's. **Round 05 (2026-09-02, the UNION of two independent gates over `0312634` — a blind Codex gate and a Fable gate — corrected by a third agent):** the cheap signal that every `search` compares covered `items.json` ALONE while the index derives from THREE inputs, so `xbrain topics` (which writes `topics.json` and never touches `items.json`) and `xbrain vocab` left `search` answering over the old topic plane with `degraded: ["no_embeddings"]` and `status` reporting `topics_changed=1` — criterion 4b passed as written and the signal covered 1 of 3 inputs (P1a; now three `stat`s, `Config.vocab_path` names the third file, a pre-round-05 manifest reads its missing entries as zeros and is declared behind until one `update`). And the manifest was sealed with a `stat` of `items.json` taken AFTER the commit, so a save landing between the CLI's load and that stat certified a store the base had never seen: `raceonlytoken` in the file, not in the base, no `index_behind_store` (P1b; `load_index_inputs` reads each input through its own handle, takes `fstat` BEFORE reading and hands the signal over with the objects — `build`/`update` seal THAT one, and the store's atomic `os.replace` is what makes the handle's bytes and its stat describe one inode). From the Fable gate: the profile plane of `search` could disappear entire with 274 tests green — `search vgonpa` on the fixture went from 10 profile-only results to 0 with nothing red — and has its positive service test now (A-1); `search` never checked `chunker_params` and answered over a manifest `update` refused and `status` called unusable (M-1, `QueryContext.params` from the same `IndexOptions`); `get --surface X` returned an EMPTY bundle instead of «no tiene X» on every item with ANY failed fetch — 62 of 2,404 — and trimmed a mixed request in silence (M-2, name by name against the failed kinds' surfaces); `ESC[2K`/`ESC[1A`/BEL stored in a tweet reached the TTY through the human view and could erase the fence G-7 put there (M-3, C0 minus TAB/LF, DEL and C1 removed from every body line, title, summary and excerpt — the sequence parameters stay as inert text); `update`'s `items_added`/`items_removed` were still satisfied by a boolean (M-4, `== 2` on two added and two removed); and `status` runs `PRAGMA quick_check` (155–167 ms by the gate, a 425 ms median at load 8.6 re-measured in round 05, on the 52 MB real index) so a damaged page the open-door probes never read is declared before a query hits it (B-1). **Round 06 (2026-09-02, the UNION of a blind Codex gate and a Fable gate over `a58eb6e`, corrected by a third agent — and the round that stopped closing one route at a time):** the FAIL-OPEN family had reappeared by five routes (C-1, C-2/C-3, G-2, P1a, and now a manifest with `counts: {}`) and the ATTRIBUTION/LOCATOR family by four (search without the surfaces join, a fingerprint blind to the author, the human chunk header, and now the chunks of `get`), every fix right and every door keeping its own reading of the question — rule 5. So this round is two SEAMS, each with an identity test that swaps the seam for a sentinel and expects the sentinel from every consumer. **(a) `index_build.describe_base`** — ONE answer to «does this manifest describe this base?», asked by `status` (as advice), `search` and `update` (as the refusal): `quick_check` first for the two maintenance doors (`update` pays it now too, because it re-seals the manifest; measured, `update --dry-run` returned a normal report over a `chunks` root page of `0xff`), the five `COUNT(*)` against the five REQUIRED planes, and every `sqlite3.DatabaseError` of a maintenance read converted into the rebuild advice (`index_schema.reading_base`). It closes **B1** (Codex: `Manifest.from_dict` validated the top-level keys and cast the rest, so `counts: {}` loaded, the consistency check iterated what the manifest offered and compared NOTHING, and `status`/`search`/`update` converged on an amputated base as healthy — the nested schema is now total and closed: `COUNT_PLANES`, `SKIPPED_CAUSES`, `CHUNKER_PARAM_NAMES`, the `StoreSignal` fields, non-negative ints only, and `write_manifest` round-trips through the reader) and **D-1** (Fable: the root page of `items` was a 61-line raw traceback on the three commands while THIS FILE said G-4 had closed it on all three — corrected above), and **F-1** got its test (a pre-round-05 manifest is declared behind until its first `update`; the one green mutation of 30). **(b) `chunking.fragment_locator`** — ONE narrowing of a surface locator to a fragment, used by `_chunk` and by `search._match`: **B2** (Codex: a `KnowledgeChunk` had NO locator and carried the item's URL, exactly when `get` delivers chunks and the surface holding the locator is not in the bundle; `KnowledgeChunk.locator` is now required — a field spec §3.7.2 demanded from the start; round 06 called it «an additive key on `schema_version: "1"`» and round 07 (U-1) bumped `EvidenceBundle` to `"2"`, because under `extra="forbid"` a required key is not additive for the only consumer that exists), **B-k** (a hit with no resolvable surface row is excluded and counted, never given a fabricated locator), and on the human side **D-3** (`render._author_label`, the one way an author is printed: a newline in `author.name` forged a header and a fence at column 0 and an ESC in the handle reached the TTY — M-3's residue, declared smaller than it was; security, not cosmetics). **S-7** got its test (`get`'s `verification` vanished with 443 green), **N-1** its counter (`chunks_deleted` omitted the topic plane), **N-4** its split (`_select` was the one radon C in `knowledge/`, introduced in round 05 and reported as pre-existing), and **B3** its table (the baseline's PROVENANCE axis was computed and never published: 23 `construido`, `real` sin cobertura — the number Plan 03 has to beat is a mean over constructed questions). **Round 07 (2026-09-02, the UNION of a blind Codex gate and a Fable gate over `9dfa34e` — both confirming the two seams WORK, both finding consumers that walked around them):** the round that stopped adding sentinel tests and started ENUMERATING consumers — `tests/test_knowledge_seams.py` reads the source with `ast` and goes red when a function opens the base without asking both halves of seam (a), or builds a served fragment without `fragment_locator`, or hashes a chunk without `chunk_evidence`; the human view's enumeration is the CONTRACT's own string-field partition, forged in totality. **U-1** (both gates): `KnowledgeChunk.locator` required under `schema_version: "1"` was a silent break — measured against the exact `origin/develop` model, the version-1 consumer refuses a bundle carrying the key and the new consumer refuses a version-1 document — so `EvidenceBundle` is at `"2"`, the policy is written once in `contracts.py`, and `knowledge inspect` reads the number off the model. **U-2** (both gates): `status` tested `exists()` by itself and read a standing manifest over a missing base as an index never built (`incomplete: false`, «+2404 nuevos», advice `index update`, which then refused) — it asks `require_database` now, like every door. **U-3** (Fable): the URLs of the human view were the FOURTH family printed raw (bodies → titles → authors → URLs), and `Item.url` is built from the handle D-3 fenced — `render._label` is the one way a non-body field reaches a human, and every string field of the contract is forged at once. **U-4** (Codex): the door verified TABLE names, so `ALTER TABLE surfaces DROP COLUMN attribution_name` (`quick_check: ok`) passed `status` and `update` and killed `search` late in a raw traceback — the reference is now the DDL itself read back through `PRAGMA table_info` (name, type, `NOT NULL`, the primary-key flag that mechanises m1), and every non-parser `OperationalError` at query time is the rebuild advice. **U-5** (Codex, the one that matters): the chunk fingerprint hashed `(surface_id, chunk_index, text)` and the index served provenance, owner, position, attribution and locator beside the text bound to NOTHING — a quoted post rewritten as the poster's own `summary`, `origin: llm`, `@vgonpa`, a valid URL to the poster's page, served with `corrupt_chunks_excluded: 0`; `chunking.chunk_evidence` is now the ONE projection the emitter hashes and the verifier rebuilds from the served row (the locator narrowed through `fragment_locator`, so seam (b) IS what integrity checks), eleven per-field mutations pin it, the counter is asserted exact (F7-6: `int(bool(…))` had kept 495 green against `>= 1`), and the physical schema is **3** (a v2 base's fingerprints were text-only, so the door refuses it by name). **U-6** (Codex): the evaluation depth was counted in CHUNKS and every metric in OWNERS, so `recall@10` for the real case F2 was 2/3 with `--k 10` alone and 1.0 beside `k=20`, and the sweep ignored `--limit` (byte-identical reports at 10 and 150) — the depth is in owners now, published on every report, and the regeneration RETRACTS the round-03 claim that 800/0 beat 1200/150 on `recall@10` (both 0.8264 in owners; the winner holds by MRR 0.8179 vs 0.7449 and `recall@1`), moves `precision@10` 0.3152 → 0.3087 (ten owners in the denominator whenever ten exist) and leaves every `recall@k` unchanged. Also: `tokenize`/`connective` compared by every door (F7-8), `reading_base` with its own test (F7-5), the cost table re-measured (F7-4), and `producer` on ASR/VLM surfaces DECLARED as the configured command rather than the one that wrote the text (F7-7 — a store change, outside Plan 02, left as an open issue; round 08 stopped emitting the claim altogether: `producer: None` on `video_transcript`/`video_frame`, the emitter takes no command, 0 fingerprints moved, and recording the transcriber on the `x_video` source is Plan 03's issue). **Round 08 (2026-09-02, the UNION of a blind Codex gate and a Fable gate over `36f694b` — both confirming rounds 01–07 stand, both finding the guard that claimed to close the families measured the literal form):** **A-2** (Fable, the gravest of the branch): `index_build._read_bound` caught EVERY `OSError` and returned the empty reading reserved for an ABSENT file, so an `items.json` with `chmod 000` — or a directory standing in its place — loaded as an EMPTY store: `status` healthy with `items_removed 2404`, `search` 0 results exit 0 declaring nothing, `update` planning `-2404`, and **`build --force` replacing 22,286 chunks with the topic plane's 703, sealed consistent, exit 0** — the fail-open family back whole through the loader, the route BEFORE seam (a), with a destruction on top. `except FileNotFoundError` now (as `load_store` always did); every door exits 1 naming the file and the index on disk is byte-identical. **A-1** (Fable): `tests/test_knowledge_seams.py` was red on the two literal forms and green on nine ways around them (`model_validate`, `model_construct`, `model_copy(update=)`, a class alias, `sqlite3.Connection`, `getattr`, a lambda, an `async def`, a helper in `cli.py`, a subpackage); it now resolves every REFERENCE through imports, aliases, same-module definitions and re-exports over every module of `src/xbrain` in every scope, polices the Pydantic constructors, `model_copy`, `hashlib`, a discarded `describe_base` verdict, `getattr`/`import_module` over a watched module, and checks that each builder's `locator=` IS a `fragment_locator` call and each hasher's argument IS a `chunk_evidence(...)` call (the dead-call forms); the gates' 32 bypass forms live IN the file as positive controls, and the docstring says what it cannot see (values behind `getattr`, `exec`, a chunk copied outside the package — held by the behavioural sentinels). **M-1** (Fable): `knowledge inspect` was a third human view of the contract printing author/URL raw (pty: `ESC[2K` ×3, forged header at column 0) — `render.render_inspect`, behind the same labels, forged in the totality test. **M-2** (Fable): a forged `chunk_id` was served with `corrupt_chunks_excluded 0` — `verify_fingerprints` recomputes it through `ids.chunk_id`. **M-4** (Fable): `SearchResponse.truncated`/`cursor` were never set (`--limit 2` cut fifty results in silence, spec §9.3) — `search` pages by `s:<offset>` over a window one owner deeper than the page, materialised by `LexicalIndex.search_owners`, the U-6 loop MOVED out of `evaluation` so the harness and the service share one definition; the human view prints a runnable continuation with every filter. **T-1** (Fable): the manifest's version strings reached the terminal raw through the incompatibility sentence — `!r`, and `render_status` labels its advice. **F7-7 / Codex F1** (the discrepancy): the configured transcribe/vision command was served as an ASR/VLM surface's `producer` — configuration presented as provenance; now `producer: None` on those two surfaces, the emitter takes no command, no fingerprint moved, and recording the transcriber on the `x_video` source is Plan 03's issue. **S-1/S-2/C8-2** (docs): the sweep's tie-break is `recall@k` → MRR → fewer chunks, written in the plan, the README and the report (which names the criterion and publishes `recall@1`); this file's sweep bullet no longer affirms the retracted advantage; the README's `0.26 s` is `0.57 s`; the «frozen at "1"» residue reads per envelope. Real corpus byte-identical before and after (sha256 `f76341a3…`); `data/index` never created in the repo.
- **The chunker's parameters are MEASURED now, and the overlap axis was decided by a retriever
  ...
  provisional `1200/150` to **`800/0`** and bumped `CHUNKER_VERSION` to `v2`. **Counted in
  OWNERS (U-6, round 07) the sweep is FLAT on `recall@10` between `target=800` and
  `target=1200`**: 0.8264 at every overlap, cell for cell on every stratum. The round-03 claim
  of a `recall@10` advantage — 0.8119 against 0.8027, «`enterrado` +2.1 pp, `semantico` +2.4 pp,
  `cruzado_idioma` +0.9 pp» — was an artefact of scoring at a depth of ten CHUNKS and is
  RETRACTED; do not quote it (this bullet affirmed it in the indicative six lines before
  retracting it until round 08, S-2). What decides is rank position: MRR **0.8179** against
  0.7449 (1200/150) and 0.7667 (1200/0), and `recall@1` **0.6034** against 0.4730 — one chunk is
  always one owner, so `recall@1` does not depend on the depth. The two larger targets lose on
  recall as before. The cost is +21.6 % chunks (18,320 → 22,286). **The tie-break that decides is
  `recall@k`, then MRR, then fewer chunks (S-1, round 08).** Plan 02 §7 and the README said
  «fewer chunks» — which, after the owners tie, would have chosen 1200/0 (18,036 chunks) — while
  `evaluation.sweep_chunker` had ordered MRR before the chunk count since before any measurement
  existed; the published winner contradicted the published rule. The rule that stands is the
  code's, written now in the plan (dated amendment) and the README with its reason — `recall@k`
  and MRR are retrieval QUALITY, the chunk count is a COST, and a cost breaks a tie in quality
  only when quality is flat — and the sweep report names which criterion decided and publishes
  `recall@1` on every row (`docs/knowledge-index.md` carries the regenerated table). The winner
  is unchanged. **Overlap applies only to `video_transcript` and exists so a sentence spanning a
