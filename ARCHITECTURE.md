...
- `x_article` — an `x.com/i/article/...` long-form post, fetched via Playwright-rendered HTML. Its `ContentSourceSuccess` can carry an additive ordered `blocks` body (text + inline images) so the Article renders as a blogpost — see data-model invariant #12; the model seam is in place (#39 PR 1), the fetch/media/generate wiring lands in the later #39 PRs (until then `blocks` is `[]` and this path is text-only as today).
