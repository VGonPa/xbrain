...
**Reads.** `data/items.json`, `data/vocab.yaml`, `data/jev/topics.json`, and — for the `nota ↗` deep links only — the note paths under `<output_subdir>/items/`, probed for existence so a link is never written to a note `xbrain generate` has not produced. That probe is also the 0.45 MB of `obsidian://` paths in the page-size figure below. **Writes.** `data/jev/topics.json`, `data/jev/topics-report.{json,md}`, `<output_dir>/jev.html`, and — on a `--force` run that re-asks a current record — `data/jev/topics.<UTC stamp>.bak`.
