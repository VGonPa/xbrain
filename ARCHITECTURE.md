...
**Page size.** `jev.html` carries no charting library and no image bytes: one JSON blob with a card per post in the corpus (the whole text, up to four media paths relative to the page into `_media/` or the reason there is none, the quoted post and each evidence surface cut at 600 characters — the tweet, author and quoted surfaces point at the card instead of repeating it — photo captions at 280, the link card, enrich's topics, the card's filter keys and topic slugs, and for an evaluated post its Jev-vs-enrich rows and cost; the notes directory ships once) plus the run history. Measured 2026-09-26 on the real vault: **3,762,648 bytes** for 2,609 posts (293 evaluated; the Topics tab's confusion counts ~18 KB and `post_sets` ~23 KB, ~80 bytes per evaluated post) — about 1.4 KB per post, ~2.7 KB evaluated and ~1.2 KB not — so a fully evaluated corpus comes to about **7.2 MB** (post lists included). `test_an_evaluated_card_stays_within_its_byte_budget` pins one evaluated card under 3 KB. The browser draws 50 cards at a time and more on scroll.
