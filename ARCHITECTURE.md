...
- **Transient retries** — items whose only previous failures were `timeout` / `dns_error` are re-fetched on the next run without `--force`. Terminal failures (`not_found`, `paywall`, `forbidden`, `js_required`, `empty_content`, `blocked_interstitial`) stay skipped until `--force` — or until `fetch --retry-failed`, which targets exactly the ones a retry could repair.
...
**The flag count is an upper bound, not a census.** Re-derived on 2026-08-30: **707 of 2,404 items are flagged** — but `looks_truncated` decides on **length alone** (≥274 characters of prose unconditionally; 265-273 only when the text does not end on a terminator, and `:` / `;` deliberately do not count as terminators). It is biased towards flagging on purpose, and its docstring says why: a missed truncation reproduces the very fabrication the detector exists to catch, while a false flag costs one re-fetch. So a long, complete post is flagged too. How many? The payload store answers it directly, without a length proxy: of the **702** flagged items whose payload is on disk, a re-parse yields a **longer** body for **214** — those are genuinely short a body — and the **identical** body for **480**, which are complete and flagged only for being long. Read `707` as the work list, `214` as the measured defect floor, and the 5 unbacked items as the only ones that certainly need the network. (A cruder proxy points the same way: 146 of the 707 are under 290 characters of raw stored text, or 359 measuring the body the detector actually judges, with the trailing `t.co` stripped as `looks_truncated` strips it. The payload comparison is the better instrument and it needs no threshold.)
