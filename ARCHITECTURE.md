...
**707 is a work list, and a third of it is free.** Re-derived on 2026-08-30: **707 of 2,404 items are flagged**. `looks_truncated` decides on **length alone** — ≥274 characters of prose unconditionally, 265-273 only when the text does not end on a terminator, with `:` and `;` deliberately not counting as terminators — and it is biased towards flagging on purpose. Its docstring says why: a missed truncation reproduces the very fabrication the detector exists to catch, while a false flag costs one re-fetch. Read `707` accordingly, as work to be triaged rather than a defect rate. On **this** population, though, the bias does not appear to be firing at all: re-parsing every stored payload, the parser reports all 702 checkable items as arriving truncated with the long-form body unavailable. There is no measured population of "long but complete" false flags here.

**Having a payload is not the same as being repairable, and the split is measured.** The payload was captured at ingest, so it carries the long-form body only if X served one at that moment; where X did not, a re-parse cannot invent it. Of the 702 flagged items with a payload, a re-parse yields a longer body for **214** — repaired offline, no network. For the other **480** the payload lacks the long-form body too, and the dry run says so explicitly on each. Those 480, plus the 5 with no payload at all, still need the network. So `reextract` clears about a third of the list, and `refetch-truncated --apply` is the path for the remaining ~485.
...
| Text truncated at ingest | items `items_needing_refetch` flags | 707; 702 have a stored payload but only **214** are repairable offline by `reextract` — the other 480 payloads lack the long-form body too, so they still need the network, as do the 5 with no payload |
