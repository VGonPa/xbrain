...
**The run log.** The side-car holds only the latest answer per item, so it cannot say what Jev has cost over time. `data/jev/runs.jsonl` (`Config.jev_runs_path`) can: `jev topics` appends one frozen `jev.models.JevRun` per pass that **sent** at least one request — start and finish in UTC, `requests` (counted at the client seam by `jev.client.CountingJevClient`, because nothing else observes a send; retries inside the vendor SDK are invisible and not counted), `ok`, `failed`, input tokens per answering provider, answers without usage, models, `interrupted`. It is appended from the `finally` of `jev.run.run_topics` — THE paid pass, which `jev topics` wraps and prints for through hooks, and which a local server can call unchanged — so every exit path of a paid pass lands in it — success, partial failure, the all-failed `JevError`, a side-car save that failed, Ctrl-C — and a log that cannot be written is handed to the caller's `on_logged` hook (the CLI prints the line to stderr) instead of replacing the run's verdict. It stores **tokens, not dollars**: `jev.report.run_history` prices each pass at read time through `defaults.tokens_cost_usd`, the same formula a stored assessment is priced with, so a price correction reprices the whole history. `store.load_runs` refuses a corrupt line by path and line number, never skips it. Append-only, not snapshotted, gitignored with `data/`, like the side-car.
...
**Page size.** `jev.html` carries no charting library: one JSON blob with a row per compared post (a 240-character snippet, enrich's topics with their probabilities, the topics Jev would add, both primaries, the post's cost) plus the run history. Measured 2026-09-26 on the first paid run, 20 posts: **55,829 bytes**.
