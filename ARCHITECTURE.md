...
**Reads + writes.** Reads `data/items.json`; writes `data/verify-report.json` + `data/verify-report.md` (the markdown leads with the FAIL/REVIEW verdicts + their flagged claims; clean passes stay in the JSON). **It never mutates the store and never snapshots** — the report is derived output, and nothing reads it back.
...
| `verify-report.{json,md}` | JSON + Markdown | The LLM-as-judge verification report — one aggregated verdict (PASS/REVIEW/FAIL + faithfulness + adherence) per `(item, target)`, with flagged claims. **Report only** — derived from the judge worksheets, not part of the store; nothing reads it back | `verify` |
