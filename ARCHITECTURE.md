...
**Page size.** `jev.html` carries no charting library: one JSON blob with a row per compared post (a 240-character snippet, enrich's topics with their probabilities, the topics Jev would add, both primaries, the post's cost) plus the run history. Measured 2026-09-26: **55,829 bytes** at 20 posts, **120,670 bytes** at 99 — about 0.8 KB per post, so roughly 2.2 MB for the ~2,600-post corpus. Every matching row is rendered; there is no pagination.
