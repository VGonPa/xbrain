...
**Reads.** `data/items.json`, `data/vocab.yaml`, `data/jev/topics.json`, `data/jev/runs.jsonl` (`jev report`, `jev dashboard`), and — for the `nota ↗` deep links only — the note paths under `<output_subdir>/items/`, probed for existence so a link is never written to a note `xbrain generate` has not produced. **Writes.** `data/jev/topics.json`, `data/jev/runs.jsonl` (append), `data/jev/topics-report.{json,md}`, `<output_dir>/jev.html`, and — on a `--force` run that re-asks a current record — `data/jev/topics.<UTC stamp>.bak`.
...
**Page size.** `jev.html` carries no charting library and no image bytes: one JSON blob with a card per post in the corpus (the whole text, up to four media paths relative to the page into `_media/`, the quoted post and each evidence surface cut at 600 characters, photo captions at 280, the link card, enrich's topics, and for an evaluated post its Jev-vs-enrich rows and cost) plus the run history. Measured 2026-09-26 on the real vault: **3,689,827 bytes** for 2,609 posts (293 evaluated) — about 1.4 KB per post, ~2.9 KB evaluated and ~1.2 KB not. The browser draws 50 cards at a time and more on scroll.
