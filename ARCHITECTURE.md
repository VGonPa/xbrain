...
**Rate limiting stops the run rather than pushing through it.** A 429 on any tab parks *every* tab for one randomized 60-180 s backoff — re-checked under a lock, so one rate limit costs one backoff and not one per tab — reusing the budget the timeline scroll already uses. Past `MAX_RATE_LIMIT_BACKOFFS` the pool raises `RefetchRateLimited` instead of continuing to poke a limited endpoint, which is what escalates to a suspension. Everything repaired before that point is already checkpointed, so stopping loses nothing.
...
**The stage.** `digest.digest_videos(store, item_ids, *, force=False, keep_transcript=False, fetch_fn=fetch_videos, transcribe_fn=_default_transcribe, temp_root=None, visual=None) -> DigestReport` orchestrates, per selected video: **ephemeral fetch** (reusing PR1's `video_fetch.fetch_videos` into a `TemporaryDirectory`) → **external transcribe** (or reuse, with `keep_transcript`) → **attach** (`digest.attach_transcript`) → **discard** the bytes. Selection is `--ids` / `--topic` / `--all-pending` (resolved via `list_video_entries`), scoped by `--source`, bounded by `--limit`. **Reads + writes** `data/items.json`.
...
- **Classifies** the frame set as `slides` vs `talking_head` (`video_frames.classify_visual`) from the fraction of frames with high **edge density** (text/sharp lines → high FIND_EDGES energy; smooth faces/bokeh → low). **Slides are described**, with or without speech. What a non-slide verdict means depends on whether the transcript carries words (`digest._carries_speech`: `has_speech` **and** non-blank text, the same test every consumer of the source applies): a non-slide video **with speech** is a talking head — it **skips** the visual layer and **logs the reason** (`"visual layer skipped (talking-head)"`), since the transcript already carries it and no vision call is wasted; a **silent** non-slide video (a screen recording of a light UI, a chart, a robot, an animation, a GIF — all score below the slide edge threshold) is described as **`footage`**, capped by `[frames].footage_max_frames`, because its frames are the only evidence of what it shows. Skipping it would attach a source with no words and no frames.
- **Describes** each kept frame (a slide, or a frame of silent footage) via the **external** vision model (`vision.describe_image`, a subprocess on `[vision].command`; mirrors `transcribe.py` — no bundled default, `VisionNotFound` on a missing/unconfigured binary aborts the run, exit-0-empty is a `VisionFailed` not a silent empty). The descriptions are recorded on the `x_video` source's `frames` list; the frame **images** are persisted under `data/media/<id>/frames/<n>.png` so `generate` mirrors them into the vault's `_media/` tree and embeds them exactly like downloaded photos. All non-kept frames are discarded (ephemeral, reclaimed by the enclosing `TemporaryDirectory`).

A per-video `FrameExtractionFailed` (bad mp4) or `VisionFailed` drops the visual layer for that video (logged) while the transcript still attaches — the audio digest is independent of the visual layer. A missing ffmpeg (`FrameExtractionToolNotFound`) or missing/unconfigured vision binary (`VisionNotFound`) is a global config error that aborts the run, exactly like a missing transcriber. A silent slide deck (no speech) still gets its slides — that is where a screen-only video carries its content — and a silent non-slide video gets its frames as `footage`. Every item this run attaches with no words and no described frames, whatever the reason the frames are missing (no `--frames`, a failed extraction or vision call, all-unreadable frames), is counted in the summary's `Huecos (sin voz ni frames): N`, printed when N > 0, so a hollow entry is never silent; `--frames --force --keep-transcript` re-digests such items with each item's own stored transcript (`digest._stored_transcript`; `digest._transcript_batches` never hands one item's transcript to another), so the recovery never re-runs the ASR on an item that has one. Like any forced re-digest it replaces the source, which clears the long-form `digest` and bumps `content.fetched_at`, so `video-digest` and `enrich` run again for the item. This replacement is deliberately non-transactional with the prior visual result: once fetch succeeds, extraction/vision failure or a talking-head reclassification leaves no frames and clears the old digest. The CLI snapshot `pre-digest-video` manages the store files `items.json`, `state.json`, `vocab.yaml`, and `topics.json`: it restores those present in the snapshot (and removes a live one absent there), but never contains `data/media/` or frame PNGs. The selected items' `data/media/<id>/frames/` directories have already been deleted or overwritten by the time it is taken. Before re-digesting items that currently have frames, copy those directories aside and restore them with the snapshot if needed. Without that copy, restored metadata can point to missing files or different pixels. Hollow items have no prior frames and are not exposed to this loss. A failed fetch or transcription attaches nothing (it counts under `fallidos`), and an item already carrying a source is skipped, not recounted.
...
| `footage_max_frames` | 6 | Cap for **silent** non-slide footage, applied **after** dedup. Must be ≥ 1 |
...
The pipeline is **extract → dedupe → cap**, and **dedupe is the real reducer**: it drops the near-identical frames a held slide produces, so the budget is spent on *distinct* slides. `max_frames` is a ceiling, not the selection mechanism. Silent footage goes through the same dedup and then its own, smaller cap (`footage_max_frames`): a handful of frames covers what a silent clip shows, and each one is a vision call, so trimming there is the intended budget. That cap is logged at INFO; a slide deck that loses distinct slides to `max_frames` is logged at WARNING.

**Dedup by video identity.** The full mp4 URL is unstable (`?tag=` + rotating signing/filename), so the dedup **`VideoKey`** is the stable id parsed from the URL *path* — `amplify_video/<id>` (or `ext_tw_video`/`tweet_video`), with a query-stripped `<netloc><path>` fallback for an unrecognised pattern. `digest.group_items_by_video` groups the selection by that key. On the default path each video is fetched + transcribed **once** and the resulting source is attached to **every** referencing item (`digest.attach_transcript` returns the count). Under `keep_transcript`, `_transcript_batches` partitions one `VideoKey` by the complete stored transcript tuple `(text, has_speech, language, title)`, plus one batch for items with no stored transcript: each batch fetches once, stored batches do no ASR, and the missing batch does one ASR. The extra fetches preserve per-item transcript identity.
...
**Snapshot trigger.** `digest-video` is destructive (it rewrites `items.json`), so it **auto-snapshots** the store files in `data/` (label `pre-digest-video`) before the store write — but only when it is about to write (a pure already-digested / no-fetchable-video run attaches nothing and takes no snapshot). A snapshot failure propagates and aborts before any `items.json` write, although frame files may already have been deleted or overwritten. A later global abort can likewise leave frame files changed without any snapshot having been taken. Idempotent: an item already carrying an `x_video` source is skipped unless `--force` (which replaces the stale source in place).
...
**What it does.** `generate` writes a self-contained interactive `dashboard.html` into the vault alongside the notes, and `_index.md` links it. Everything is inlined — the data as a JSON blob, ECharts vendored from `src/xbrain/resources/echarts.min.js` (1.0 MB) into the page. **No external scripts, no CDN for code, no build step:** ECharts and the data are inlined. Two things are still fetched at runtime — the Google Fonts stylesheet and the X video posters in the media panel — so offline the file renders in system fonts with the posters hidden, rather than not rendering. (`jev.html` carries the fonts reference and no posters.) Measured on the current vault render: 1,825,197 bytes, about 1.8 MB. Its sibling `jev.html` ([§ jev](#jev)) is built by the same mechanism without ECharts; it links its photos by relative path into the vault's `_media/` mirror instead of inlining them — so it shows photos only after `xbrain generate` has mirrored them — and measured **3,762,648 bytes** on 2026-09-26 (2,609 posts, 293 evaluated).
...
**Currency is a contract, and it is the whole staleness story.** `contract` = sha256 over the contract version, the state **as sent**, and `questions_digest` — the canonical JSON of every question's type, instructions and criteria. Hashing the questions rather than the vocabulary they were built from is what removes the hand-maintained list of "things that ought to invalidate": a reworded instruction, an edited description, a new or removed topic and a different fallback all move the digest on their own. Two things are deliberately outside it — option **order**, because the question set is canonical, and the **judge**, because `provider`/`model` are recorded but never hashed, so re-pointing `[jev].model` does not retire stored work. So: re-enriching an item keeps its assessment current (the comparison is recomputed at report time — that is the event this layer exists to look at); new evidence, a moved vocabulary or a changed fallback retire it. `output_fingerprint` records which `enrich` assignment existed at ask time and is **informational only**; consulting it for currency would retire an assessment the moment the item was re-enriched. Retired records are excluded from every reader and **counted** (`assessments_stale`, `assessments_orphaned`), because a full side-car a vocabulary edit just retired must never render identically to one nobody ever wrote — the second reading sends an operator to re-pay for the corpus. The same rule governs `snapshot restore`: a restore reverts **`vocab.yaml` as well as `items.json`**, and the contract hashes the vocabulary-derived questions digest, so a restore from before a `vocab --regenerate` moves the digest and retires **every record at once**, a full re-bill; only a restore that leaves both the item's evidence and the vocabulary untouched leaves an assessment current.

**The run log.** The side-car holds only the latest answer per item, so it cannot say what Jev has cost over time. `data/jev/runs.jsonl` (`Config.jev_runs_path`) can. Every pass that **sent** at least one request appends one frozen `jev.models.JevRun` (`kind: "topics"`, start and finish in UTC, `requests`, `ok`, `failed`, `unsaved`, input tokens per answering provider, answers without usage, models, `interrupted`), and every count comes from the **client seam**: `jev.client.CountingJevClient` folds every returned `JevResult` into its counters under one lock and `snapshot()` reads them consistently, because an answer is billed when it returns — whether xbrain then keeps it (`ok`), refuses it (`failed`, with the calls that raised) or, on Ctrl-C, never drains it (`unsaved`). The line is appended from the `finally` of `jev.run.run_topics` — THE paid pass, which `jev topics` wraps and prints for through hooks, and which a local server can call unchanged — so every exit path of a paid pass lands in it: success, partial failure, the all-failed `JevError`, a side-car save that failed, Ctrl-C. Building, appending and reporting the line are all caught there (the finish is clamped to the start, a hook that raises is logged), so the log step can never replace the pass's verdict or skip the client's release; a line that cannot be written is handed to the caller, which prints it for the operator to append. `store.append_run` starts on a new line after a torn tail and rolls a failed write back to the pre-write size; `store.load_runs` refuses a corrupt line by path and line number, never skips it. It stores **tokens, not dollars**: `jev.report.run_history` prices each pass at read time through `defaults.tokens_cost_usd`, the same formula a stored assessment is priced with, and counts as **outside the log** every stored assessment whose `asked_at` no logged pass's window covers (answers from before the log, from a copy of xbrain that does not write it, from a SIGTERM-killed pass), priced from their own tokens rather than dropped. Append-only, not snapshotted, gitignored with `data/`, like the side-car.
...
**Reads.** `data/items.json`, `data/vocab.yaml`, `data/jev/topics.json`, `data/jev/runs.jsonl` (`jev report`, `jev dashboard`), and, for `jev dashboard`, existence probes: the note paths under `<output_subdir>/items/` (the `nota ↗` deep links), and each photo and video frame the page could show, in `<output_dir>/_media/` and in `data/media/` — so a link is never written to a note or a picture `xbrain generate` has not produced, and a picture that is downloaded but not mirrored says so. Those probes live in `jev.dashboard.collect_jev_media` / `note_links`, called from `build_page_data`; the blob builder `compute_jev_dashboard_data` touches no disk. **Writes.** `data/jev/topics.json`, `data/jev/runs.jsonl` (append), `data/jev/topics-report.{json,md}`, `<output_dir>/jev.html`, and — on a `--force` run that re-asks a current record — `data/jev/topics.<UTC stamp>.bak`.
...
**Seams.** `jev.client` is the vendor-free contract — `JevClient.ask(state, questions)` plus a `close()` that is idempotent and may not raise a vendor type — and `jev.typesafe` is the only module in `src/` that imports `typesafe-sdk`, converting every SDK exception into `JevError`. A second provider is a second module like it, answering the same questions into the same record (hence `provider` is required and never defaulted). **ONE ASSESSMENT PER ITEM, TODAY.** The side-car is keyed by `item_id` alone, so a second judge run over the same corpus OVERWRITES the first judge's paid records rather than sitting beside them — and the contract excludes the judge, so re-pointing `[jev].model` leaves every record current and only `--force` re-asks, destroying the old work as it goes. The record is provider-agnostic so that a panel is *possible*; making it *usable* means re-keying the side-car by `(item_id, provider, model)`, which is a follow-up and not the state of this layer. `jev.report` is the ONE place the comparison with `enrich` is computed, and the one place the run history is priced: the CLI report and the dashboard blob both read through it. The page recomputes nothing — it compares at the fixed `[jev].threshold`, ships `build_report`'s summary whole, builds each post card's Jev-vs-enrich rows from that call's `ItemComparison` (whose `disagreements` is the one definition of a post's disagreement, and whose three kinds the summary counts as `posts_enrich_only` / `posts_jev_only` / `posts_primary_differs`), takes every cost from `report` (`run_history`, `post_cost_view`, `assessment_cost_usd`), reads "what Jev saw" from `assess.state_surfaces` (the state `build_topic_state` sends, split back into its surfaces), decides in Python which filters each card belongs to (its `in` keys, from the same `ItemComparison` predicates the summary counts — `posts_with_disagreement`, `posts_enrich_only`, `posts_jev_only`, `posts_primary_differs`, `primary_fallback`, `items_unassessed`), orders the cards in Python, and only tests membership, searches and re-sorts in the browser. **The rule for the tabs:** every number a tab shows about the comparison — confusion pairs, per-topic verdict counts, and (for Comparar) confidence bands of the disagreements — is a `report.py` output, tested there, and reaches the page in the blob; the page does not tally cards into a comparison number (it still counts what it lists, e.g. «mostrando N», «ver todos (N cruces)»). **Counts in the summary; the post lists behind them in one report-built index** (`report.post_sets`, keyed `enrich~jev` by `report.pair_key`), which `build_page_data` ships and `write_reports` does not — the lists grow with every evaluated post, the report file is for numbers. The Topics tab reads `per_topic` (with `enrich_primary` / `jev_primary`), `topic_confusion` and `primary_confusion`, opens a pair through `post_sets`, and groups a topic's cards by the verdict of that topic's row, which Python wrote. The floor under which a topic is «pocos datos» is `dashboard.TOPIC_MIN`, shipped as `topic_min`. The Comparar tab reads the same summary plus `per_topic.primary_both` (the diagonal of the primary cross) and `confidence_bands` (how sure Jev was on each disagreement: pairs and posts per band); the band EDGES are defined once in `report._BAND_SPECS`, cut at the threshold by `report.confidence_bands` and shipped with each band (with its `label`, part of the report contract, and the summary `total` it splits), and `report.band_of` refuses a probability no band holds instead of dropping it. The threshold the bands are cut at is `ItemComparison.threshold`, set by `compare_item`: `post_sets` reads it off the comparisons and refuses a list made at two thresholds, so no second call site is handed one. `post_sets` indexes `cx`/`px` by pair, `pd` by topic and `bands` by band key; the tab takes its topic order from the Topics tab's `defaultTopicOrder` and its kind counts from the Posts filters, so a number there is always the same number as elsewhere on the page. The Configuración tab reads the blob's `config` block, built by `dashboard.config_view` from the sources the commands use: the `[jev]` keys and defaults from `defaults.JEV_DEFAULTS` (the list `config.py` validates against), the price from `defaults.INPUT_USD_PER_MTOK`, the questions exactly as `questions.build_topic_questions` builds them (wire order, criteria as ordered pairs) with `assess.questions_digest`, the state's surface order and cut line from `assess.STATE_SURFACE_KEYS` / `assess.CUT_MARKER` (which `build_topic_state` uses), the file paths from `Config` and `report.report_paths` (`build_page_data` passes them; a pure call ships `None`), and the pass estimate from `report.pass_estimate` over `post_cost_view` and `assess.select_items` — the template carries none of the wire text. A page that receives new data (a future local server) calls the template's `loadData(blob)` and then re-draws the current tab: it sets `DATA` and every value derived from it — `LABEL`, `DESC`, `TOPIC_MIN`, the threshold's printing precision — while the fallback filter's name, the «Jev confirma» tooltip and the whole Configuración tab read `DATA` when drawn. Nothing else is frozen at script load; the header, cost strip and three numbers are drawn by `boot` and must be re-drawn with the tab. `jev.load.load_jev_pairs` is the one loader of store + vocabulary + side-car for every reader, and `jev.dashboard.build_page_data(cfg, now=…)` the one assembler of the page's inputs; the CLI calls it, and so must any server. **Serving the page** (a future local server) must serve it from a URL whose directory maps to `<output_dir>`, because photos are relative `_media/…` paths, and should cache `collect_jev_media` (thousands of `stat`s on an iCloud vault) rather than repeat it per request. `jev.defaults` is the one place a run is priced **and** the one place the bill is worded, after three call sites formatted the same side-car as `~0.000 $` and `~0.0001 $`.

**Failure.** Per-item failures are recorded with their reason and echoed, the rest of the run is saved, and a run where **every** item failed raises rather than report an empty success. A missing key and a malformed vocabulary are operator errors raised before the first call — the key is checked before the SDK is even imported, so `xbrain --help` never loads the vendor's HTTP stack. A partial answer set (a missing Noul, a choice outside the offered options, a winner absent from or beaten inside its own distribution) is rejected, never stored. Records are checkpointed to disk every 25 (`jev.run.CHECKPOINT_EVERY`); Ctrl-C saves what was banked — `run_topics` RETURNS the interrupt rather than raising it, and the CLI turns it into exit 130 — and an interrupt that rescued nothing writes nothing — saving there would put the unchanged map over the side-car, which on a first run is `{}`.

**Exception languages.** The dividing line is the **audience**, and the exception type follows it. `JevError` (a `RuntimeError`) is the only type the seam emits and always carries an operator-facing **Spanish** sentence: an unusable key or client configuration, a provider error, an answer set this repo refuses, an unknown `--id`, a `--limit` below 1. `ValueError` is what the package raises for faults that are not the provider's — a malformed vocabulary (``el vocabulario está vacío: ejecuta `xbrain vocab`…``, a duplicate slug, a fallback colliding with a slug, a topic with no description) and an out-of-range `--threshold` — and those are operator-facing too, so they are **Spanish as well**. English survives in the operator's path in exactly one place: a `config.toml` schema fault, raised by the shared loader and worded like the rest of `config.py` (`config.toml: [jev].threshold must be a number in [0.0, 1.0]`). Some `ValueError`s are genuinely programmer-only, and English: `report.band_of` handed a probability no band holds, and comparisons made at two thresholds handed to `post_sets` or the band rows (no stored answer reaches either). So is a question map carrying no `primary` Choice, and that one is a `ValueError` rather than a `JevError` precisely because the provider did nothing wrong: dressing a caller bug as a Jev failure would send whoever reads the failure table looking at the API. Both types reach the terminal through `_handle_cli_errors` as `Error: …` with exit 1, so the split is about who the message is for, never about how it is surfaced.

**Page size.** `jev.html` carries no charting library and no image bytes: one JSON blob with a card per post in the corpus (the whole text, up to four media paths relative to the page into `_media/` or the reason there is none, the quoted post and each evidence surface cut at 600 characters — the tweet, author and quoted surfaces point at the card instead of repeating it — photo captions at 280, the link card, enrich's topics, the card's filter keys and topic slugs, and for an evaluated post its Jev-vs-enrich rows and cost; the notes directory ships once) plus the run history. Measured 2026-09-26 on the real vault: **3,799,403 bytes** for 2,609 posts (293 evaluated; the confusion and band counts ~19 KB and `post_sets` ~38 KB, ~130 bytes per evaluated post) — about 1.4 KB per post, ~2.7 KB evaluated and ~1.2 KB not — so a fully evaluated corpus comes to about **7.3 MB** (post lists included). `test_an_evaluated_card_stays_within_its_byte_budget` pins one evaluated card under 3 KB. The browser draws 50 cards at a time and more on scroll.
...
| `jev/topics.json` | JSON dict of `TopicAssessment` | The **Jev side-car**: one topic assessment per item id — the membership probability per vocabulary slug, the primary Choice with its distribution, the provider and model that answered, and the `contract` that says whether it still describes today's question. **Not part of the store, and not snapshotted** (see invariant 8): `xbrain jev` never writes `items.json`. Rewritten wholesale on every save, atomically, sorted. It costs money to regenerate and `snapshot restore` does not reach it, so a malformed file raises rather than reading as empty; `jev topics --force` copies it to `jev/topics.<UTC stamp>.bak` before re-asking a current record | `jev topics` |
| `jev/runs.jsonl` | JSON Lines of `JevRun` | The **Jev run log**: one line per `jev topics` pass that sent a request — `kind` (`topics`), requests, ok, failed, `unsaved` (answered but not saved when Ctrl-C landed), input tokens per provider, models, interrupted, start/finish in UTC. Tokens, never dollars (priced at read time). **Append-only, not part of the store, not snapshotted**; a corrupt line is refused by number. Read by `jev report` (the `Histórico:` line) and `jev dashboard` (the cost strip) | `jev topics` |
| `jev/topics-report.{json,md}` | JSON + Markdown | The comparison of that side-car against `enrich` at one threshold — both directions of agreement, the doubtful pairs, the missing candidates, the never-asked slugs, the primary ranking, and the bill. **Report only, and nothing reads it back**; the dashboard computes through `jev/report.py`, not through this file. The markdown is truncated to the worst 20 rows per table and says so; the JSON carries every compared item | `jev report` |
...
14. **One definition of evidence, shared by every component that needs it.** `evidence.evidence_surfaces(item, target)` is the single source of truth for what may support a claim in a generated output, and it is **target-scoped**. The generator ships at least those surfaces, the judge's source is exactly them, `rubric-verify.md` declares every one of them, the entity checker searches `evidence_text` — the same surfaces' atomic values, labels stripped — and [`jev`](#jev) builds its state from the same surfaces' values, in its own rendering (post first, no labels, end-cut). That is **five** consumers, one definition. No component keeps its own list: `tests/test_evidence_contract.py` binds the first three by identity against the shared function, and `tests/test_checker_evidence_binding.py` binds the fourth through the checker's public scan. A link is never a surface. Hand-written lists are how the judge came to be handed the linked article for a digest whose generator never saw it, excusing inventions it could not have sourced, while a green suite reported agreement.
...
│   │   ├── run.py           ←   run_topics: THE paid pass — backup, checkpoint, save, run log, teardown
│   │   ├── store.py         ←   data/jev/topics.json (atomic, sorted, never items.json) + runs.jsonl (append)
│   │   ├── load.py          ←   load_jev_pairs: store + vocab + side-car, current pairs, counted
│   │   ├── report.py        ←   THE comparison with enrich at a threshold (+ the markdown)
│   │   └── dashboard.py     ←   the jev.html blob (pure) + collect_jev_media / build_page_data (IO)
...
│
├── .env                     ← TYPESAFE_API_KEY for `xbrain jev` (GITIGNORED, never committed)
├── .env.example             ← the committed template; carries no value
│
