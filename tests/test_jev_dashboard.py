...
def _compare_fixture() -> dict[str, Any]:
    ...
    Bands (pairs/posts): e-lo 5/4 (l00 has two) · e-mid 3/3 · e-near 4/2 (n00, n01 two each) ·
    j-near 22/22 (past one page of 20) · j-hi 2/1 (h00 two). Disagreement kinds (posts):
    enrich asigna y Jev no 9 · Jev añadiría 23 · primario distinto 10. Primary cross, off the
    diagonal: delta~otro 4 · gamma~omega 3 · -~alpha 2 (no enrich primary) · alpha~beta 1;
    diagonal: alpha 28 · beta 4 · gamma 2. «otro» 4. Default topic order: gamma (0 %),
    delta (80 %), alpha, then beta under the floor, then the never-assigned: 12 topics.
    Plus two never-asked posts."""
...
def test_the_compare_fixture_tells_every_number_apart():
    ...
    assert (s["posts_enrich_only"], s["posts_jev_only"], s["posts_primary_differs"]) == (9, 23, 10)
    ...
    assert both == {"alpha": 28, "beta": 4, "gamma": 2}
    assert s["primary_fallback"] == 4 and len(data["topics"]) == 12
...
def test_the_fallback_posts_are_the_primary_cross_rows_that_end_in_the_fallback():
    ...
    assert sum(r["posts"] for r in rows) == data["summary"]["primary_fallback"] == 4
...
def test_the_compare_words_that_carry_meaning_are_pinned():
    ...
    for words in (
        ...
        "'probabilidad < '",
        "'probabilidad ≥ '",
        "'Ese grupo ya no existe en estos datos; arriba, la comparación completa.'",
...
def test_the_band_edges_the_page_states_are_the_ones_the_report_ships():
    ...
    assert "dec(b.lo, 2)" in compare and "dec(b.hi, 2)" in compare
