...
from xbrain.knowledge.render import render_get, render_search, render_status
...
def _forged_response() -> SearchResponse:
    match = SearchMatch(
        ...
        derived=False,
        excerpt=FORGE,
        # `SearchMatch.title` did not exist when this fixture was written: child 02.5 added
        # it to the contract AND to the partition, so the totality assertion below went red
        # here with `[('SearchMatch', 'title')]` — a field the contract declares that the
        # forge did not reach. That red is the guard working. The enumeration comes from
        # `CONTRACT_MODELS` and the partition rather than a hand-written list, so a contract
        # that grows breaks the fixture instead of quietly covering less (rule 1).
        title=FORGE,
...
# ---------------------------------------------------------------------------
# T-1 — the FIFTH renderer: `index status`, whose strings an operator edits by hand
# ---------------------------------------------------------------------------


def _manifest(**kwargs):
    """A `Manifest` as the writer seals it. Every string is a candidate for the forge."""
    from xbrain.knowledge.index_build import Manifest, StoreSignal
...
def _status_report(**kwargs):
    """A `StatusReport` over that manifest."""
    from xbrain.knowledge.index_build import StatusReport
...
def test_a_forged_manifest_string_cannot_forge_a_header_through_render_status() -> None:
    """T-1, asserted WHERE THE SENTENCE IS PRINTED — which is this module, and which is why
    `test_knowledge_index_build.py` reads the `status` door as its `advice` STRING and says
    in its own docstring that the rendering half is 02.11's.

    The manifest is a JSON file an operator edits by hand, so its strings are as untrusted
    as a tweet: a newline in `chunker_version` stands at column 0 as a renderer header and
    as a fence line, and an `ESC[2K` in `schema_version` erases the line above it. Those
    strings are NOT the contract's — they never enter `CONTRACT_MODELS` — so the totality
    forge above cannot reach them and this test is the only thing that does.

    Every string field the manifest declares AND `render_status` prints is forged at once,
    plus the `advice` sentence, which quotes the manifest's own strings back.

    Seen red by replacing `_label` with the identity in `render_status`: the forged header
    stood at column 0 and `\\x1b` reached the output.
    """
    report = _status_report(
        manifest=_manifest(schema_version=FORGE, surface_version=FORGE, chunker_version=FORGE),
        advice=FORGE,
        incomplete=True,
    )
    out = render_status(report)
    lines = out.splitlines()
    assert "\x1b" not in out, out
    assert FORGED_HEADER not in lines, out
    assert not any(line.startswith(FORGED_FENCE) for line in lines), out
    assert not any(line.startswith("[") for line in lines), out
    # The forged text is still THERE — it is what the manifest says, and hiding it would
    # leave an operator unable to see what their file holds. It just cannot stand alone.
    assert FORGED_HEADER in out


def test_a_missing_manifest_says_so_and_labels_its_advice() -> None:
    """The other branch of `render_status`, and the one a reader meets first: no manifest.

    Its `advice` is the string `load_compatible_manifest` builds by interpolating the
    manifest's own version strings, so it is forgeable by exactly the same route and goes
    through the same label. Asserted separately because the early `return` means the
    branch above never executes this line.
    """
    out = render_status(_status_report(manifest=None, advice=FORGE))
    lines = out.splitlines()
    assert "INCOMPLETO o inexistente" in out
    assert "\x1b" not in out
    assert FORGED_HEADER not in lines, out
    assert not any(line.startswith(FORGED_FENCE) for line in lines), out


def test_render_status_does_not_invent_a_tokenizer_the_manifest_never_sealed() -> None:
    """This tree's `Manifest` declares neither `tokenize` nor `connective`: 02.6b sealed it
    without them and left the two to the child that ships query semantics.

    The ported renderer printed both. Printing a DEFAULT instead would state as the index's
    own configuration a value nothing sealed — and `status` is the line a reader consults
    precisely to learn what the index was built with, so a plausible invention there is
    worse than an absence. Asserted on the absence of the labels, so the day the manifest
    grows the fields this test is what has to be updated deliberately.
    """
    out = render_status(_status_report())
    assert "tokenizer" not in out, out
    assert "conectiva" not in out, out
    assert "esquema 1" in out and "chunker v2" in out, "what IS sealed is still printed"
    assert "embeddings —" in out


def test_the_incomplete_warning_is_printed_before_the_advice() -> None:
    """C-3: an unusable index says so BEFORE the remedy, so the built-at header above it is
    not read as health. Position is the requirement, so position is what is asserted."""
    out = render_status(_status_report(incomplete=True, advice="Reconstruye."))
    lines = out.splitlines()
    warning = next(i for i, line in enumerate(lines) if "INCOMPLETO o inutilizable" in line)
    advice = next(i for i, line in enumerate(lines) if "Reconstruye." in line)
    assert warning < advice, lines
