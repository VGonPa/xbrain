...
| `index status` | What the index holds and how far behind the store it is: versions, counts, what was skipped, items added/changed/removed, topics whose membership moved, and whether the cheap staleness signal has tripped. **It is the command that answers when the others refuse** — `search`, `build` and `update` reject an absent or incompatible index; this one reports it, and names the same command they name. (`get` is not among them: it opens no database, so an absent index is nothing for it to refuse.) It also pays for `PRAGMA quick_check`, which the query doors deliberately do not, so it sees corruption on pages no query reads. `--json`. |
