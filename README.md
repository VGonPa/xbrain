...
as a talking-head: old frames and the long-form digest are removed. Restore the
automatic `pre-digest-video` snapshot if that result is worse. `--frames` is fully
opt-in — a normal `digest-video` run never touches ffmpeg or the vision model.
