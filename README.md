...
offline. Its sibling `jev.html` is built the same way, without ECharts, and references
only the fonts.
...
| `jev dashboard` | Write `jev.html` next to `dashboard.html` in the vault: one table of the posts where `enrich`'s topics and Jev disagree at `[jev].threshold` (✓ / ✗ per topic, the topics Jev would add, the primary on both sides, each post's cost), the three numbers `xbrain jev report` prints, and what Jev has cost — total, per pass from `runs.jsonl`, and per post. Computes nothing in the browser; filters and search only. Also free. `xbrain generate` links it from `_index.md` once the page exists. |
