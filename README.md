...
| `index build` / `index update` / `index status` | Build, incrementally update and inspect the persisted retrieval index at `data/index/` (SQLite + FTS5). **Derived and reconstructible** — never versioned, never a second source of truth, and losing it costs one command: a full build is ~1.4 s on a 2,400-item corpus. `build` refuses to overwrite an existing index without `--force` (and `--force` unlinks first, so the file size is deterministic rather than growing with each rebuild); `--dry-run` reports the counts it WOULD write and touches no file at all. `update` compares a per-item fingerprint over the **emitted surfaces** — not over `fetched_at`, which cannot reach the 40 % of items with no `content` — and rewrites only what moved: 100 changed items cost 0.57 s (synthetic population, re-measured 2026-09-02 after `update` began paying `PRAGMA quick_check` and the column check; it was 0.26 s before those two guards and this line stood two rounds behind the table in `docs/knowledge-index.md` — C8-2). A changed `vocab.yaml` also rebuilds the item **profiles**, because topic descriptions enter them. `update` **refuses** when the emitter, the chunker or its parameters moved, naming `index build --force`: an incremental update under a new chunker would blend two chunkers with every id still resolving. `status` loads the store and reports **how many** items changed, added or vanished — a number, not a flag — and how many topic rows hold members the store no longer implies (`topics_changed`): a topic move done by `enrich` rewrites the item AND the members and `stale` bit of every topic it enters or leaves, and until round 04 `update` rewrote only the item (H1). All three are read-only with respect to `items.json` / `vocab.yaml` / `topics.json`, and none takes a snapshot, because none is destructive. |
| `search` | Query the corpus and get items back with their **citable** fragments. All eight filters of the spec — `--from`/`--to`, `--source`/`--mine`, `--author`, `--topic`, `--kind`, `--origin`, `--has-surface` — are pushed into SQL **before** bm25 scores anything, so a filter never competes with your terms for weight. Results are grouped by item (at most 3 matches each, configurable) so one long transcript cannot occupy the top ten with ten adjacent windows of itself, and every result names the exact `xbrain get …` that pulls up the source. Two things it will always tell you: when the index is **behind the store** (it compares the mtime+size of `items.json`, `vocab.yaml` and `topics.json` against the manifest on every query — all three, because `xbrain topics` and `xbrain vocab` change what the index serves without touching `items.json` — and still answers: stale evidence is usable if it says so) and when a match on derived text leads to **no underlying source** (`verify_with: []`). Verification verdicts are hydrated from the live store, so a revoked FAIL is never shown as the old PASS. A chunk whose fingerprint does not recompute — over its text **and** the provenance, attribution and locator served beside it — is excluded and counted, never repaired: the connection is opened read-only, so a repair cannot happen. Read-only, no snapshot, no network, and **not one model call**. `--limit`, `--json`. |
| `get` | Return one item's evidence — metadata, topics, the summary, and any surface you name in full. **Reads the store, not the index**: it works with `data/index/` deleted entirely, which is the operational definition of "the index is not a second source of truth". Without `--surface` it gives you the metadata and the list of surfaces you can ask for, not a dump. With `--surface external_article` it gives you the article body **complete** — the `ARTICLE_CHAR_LIMIT` that truncates LLM prompts does not apply to retrieval. With `--query` it prioritises the fragments that score inside a long source, using the same scorer as `search`. Over the budget it paginates with an explicit `truncated: true` + `cursor` — and the human view prints the continuation WITH the same `--surface` and `--query`, because the cursor is an offset into the sequence those two define (H2) — and never cuts a surface's body mid-way (that would leave a fingerprint that no longer describes its own text); a failed fetch comes back as structured state with its `failure_reason`, and a link with no body comes back with its reason and no text. The human view names the author of every surface and chunk whose author is not the item's (a quoted post is somebody else's words, whether it arrives whole or as a chunk — H3). `--surface` (repeatable), `--query`, `--budget`, `--cursor`, `--json`. |
| `eval` | Evaluate retrieval against `eval/golden-set.yaml` (tracked in Git) and publish `data/eval-report.{json,md}` (not tracked — it carries corpus excerpts). Metrics per **strategy × stratum × provenance**, never one global figure. A stratum with no cases is reported as *sin cobertura*, never as 0.0, and a case whose filters the strategy cannot apply is reported as **not measured** rather than as a failure. With the persisted index in place the lexical baseline now applies **all eight** filters, so the `filtros` stratum publishes a number for the first time; the mechanism still guards the strategies that do not exist yet. **`--sweep-chunker target=800,1200 overlap=0,150`** scores the cartesian product of chunker parameters against the golden set and publishes `data/eval-sweep.{json,md}`, ranked by recall@k, then MRR, then fewer chunks — quality first (is the relevant item on the page, and where), the index's cost last — and the report names which criterion decided; a flat result is reported as flat rather than dressed up as a winner. *(Until round 08 this line said «tie-broken by fewer chunks» while the code ordered MRR before the chunk count and the published winner, 800/0, had been decided by MRR after a tie on recall@10 with 1200/0 — S-1; the rule that stands is the one written here, with its reason in Plan 02 §7 and `docs/knowledge-index.md`.)* **Report-only**: never writes `items.json`, never snapshots. Without `--min-recall` it reports without judging (the spec fixes merge thresholds *after* the baseline runs); with it, the command becomes a gate and exits non-zero naming the bucket that fell short — or, when the threshold could not be compared against a single bucket, naming that count, so a gate that measured nothing fails closed instead of reporting PASS. Cases that retrieved nothing at all are counted per bucket (`vacíos`), because a 0.0 from an empty result set is a different fault from a 0.0 from a bad ranking. `--strategy`, `--k`, `--golden-set`, `--report`, `--json`. |
