...
and the rest). Two more extras are optional: `[embeddings]` (`numpy`, for the
vector plane) and `[mcp]` (the SDK behind `xbrain mcp-serve`). `--index-url https://pypi.org/simple` is only needed if your
...
[embeddings]                              # optional; unset = search stays lexical
# command = "/path/to/env/bin/python /path/to/xbrain/scripts/xbrain-embed"
# model = "intfloat/multilingual-e5-base" # optional; omit for the backend default
# batch_size = 64
# timeout_seconds = 600
# query_prefix = ""                       # E5/BGE models want "query: " / "passage: "
# passage_prefix = ""
```
...
| `[enrich]` | `model` | `claude-haiku-4-5-20251001` | Model for the `api` executor. |
...
| `[embeddings]` | `command` | — (unset) | External embedding backend for the **optional** vector plane (`index build --embeddings`, `search --strategy vector\|hybrid`). No bundled default — the model is chosen by evaluation, not by a default. Unset, `search` stays lexical and says so. Needs the `[embeddings]` extra (`uv sync --extra embeddings`). See [docs/knowledge-index.md](docs/knowledge-index.md#the-vector-plane-and-hybrid--opt-in-and-not-the-default). |
| `[embeddings]` | `model` | — | Model passed to the backend (`--model`) **when building**. Queries use the model recorded in the index manifest. |
| `[embeddings]` | `batch_size` · `timeout_seconds` | `64` · `600` | Texts per subprocess call; wall-clock cap per call. |
| `[embeddings]` | `query_prefix` · `passage_prefix` | `""` · `""` | A property of the model (E5/BGE want `"query: "` / `"passage: "`). Recorded in the manifest by the build; queries read the manifest's copy. |
...
| `refetch-truncated` | List — or with `--apply`, repair — the items whose tweet text was **truncated at ingest**. `legacy.full_text` is capped at 280 chars: X cuts a long post mid-word and appends a t.co self-link, and the generator, told to summarise half a sentence, finishes the sentence itself. Unlike `reextract` this is **not** a free re-parse: `--apply` re-fetches every flagged tweet from X through a logged-in browser, whether or not its payload is on disk (the flag reads the stored text alone and never consults the payload store). Run `reextract` first — most flagged items do have a stored payload (702 of 707, measured 2026-08-30), and a re-parse repairs offline the ones whose payload holds the long body ([why, and how the 707 split](ARCHITECTURE.md#refetch-truncated)). The re-fetch is deliberately human-paced work, hours of it, checkpointed as it goes so a session that expires three quarters of the way through does not discard the three quarters already repaired. The id list is written to `data/truncated-items.json` on every run, `--apply` or not. Destructive → auto-snapshot; a repaired text invalidates its summary, so re-run `enrich` afterwards. |
...
| `index status` | What the index holds and how far behind the store it is: versions, counts, what was skipped, items added/changed/removed, topics whose membership moved, and whether the cheap staleness signal has tripped. **It is the command that answers when the others refuse** — `search` and `update` reject an absent or incompatible index; this one reports it, and names the same command they name. `build` is not among them: it is the ONE door allowed to create the database, so an absent index is the input it exists for — what plain `build` refuses is an index that already **exists**, and there it names `--force`. Nor is `get`: it never opens `data/index/` — a bare `get` opens no database at all, and `get --query` opens one that exists nowhere on disk, a scratch `sqlite3(":memory:")` over that item's own chunks — so an absent index is nothing for it to refuse either. It also pays for `PRAGMA quick_check`, which the query doors deliberately do not, so it sees corruption on pages no query reads. `--json`. |
| `search` | Ranked items with the fragments that matched, each carrying its `origin`, `trust_class` and locator — and, when the match landed on generated text, the exact `xbrain get` that fetches the underlying source. Read-only (`mode=ro`), no LLM, and no network call of its own: a `vector`/`hybrid` query may start the configured embedder, which receives the query text ([the trust boundary](docs/mcp.md#the-trust-boundary)). All eight filters are pushed into SQL **before** scoring: `--from`/`--to`, `--source`/`--mine`, `--author`, `--topic`, `--kind`, `--origin`, `--has-surface` (each repeatable where plural). Matches are grouped by item and capped at `[index].max_matches_per_item`, so a long transcript yields one result with three fragments, not ten of the top ten. Chunks whose fingerprint does not recompute, or whose surface no longer resolves, are **withheld and counted** in `corrupt_chunks_excluded`. The response declares its own degradations — `index_behind_store`, `no_embeddings`, and, when a requested vector channel did not run, `embeddings_not_configured` / `embedder_unavailable` / `vector_filters_unsupported` — rather than answering as if they did not apply. `--strategy lexical` is the default; `vector` and `hybrid` read the [opt-in vector plane](docs/knowledge-index.md#the-vector-plane-and-hybrid--opt-in-and-not-the-default). A response names `hybrid` only when the vector channel ran. `vector` with no plane or an empty `[embeddings].command` is an error (exit 1), filters or not. With both in place, a filtered `vector` request is answered lexically, declaring `vector_filters_unsupported`, because the plane has no filter columns. `hybrid_graph` answers lexically declaring `hybrid_graph_not_implemented`: the graph re-ranking is switched off (measured negative, [docs/graph-threshold-sweep.md](docs/graph-threshold-sweep.md)). `--limit`, `--cursor`, `--strategy`, `--json`. |
| `get` | One item's evidence — topics, fetch failures, unfetched links, verification verdicts, and the surfaces you ask for — **read from the live store, never from the index**, so it works with `data/index/` deleted. An index able to answer this would be a second copy of the corpus that nothing invalidates, and the day the two disagreed there would be no way to know which one the reader was shown. **A bare `get` is the index card, not the dump**: metadata, topics, the `summary` body and the *list* of available surfaces. Every other body — article, transcript, thread, frame descriptions — is asked for by name with `--surface` (repeatable; an unknown one is refused listing what the item actually has), so *complete* means every surface is reachable by selection and pagination, not everything in one call. Never truncates silently: above `[index].get_char_budget` it truncates **declaring it** and returns a cursor. That cursor is an offset into the sequence the REQUEST defined — the chunks of the selected surfaces, or their ranking under `--query` — and the bundle does not carry the request back, so **a continuation must repeat the same `--surface` flags, in the same order, and the same `--query`**; without them the cursor resumes inside the default selection or is refused outright. The truncation line prints the whole command, flags and all. `--query` to prioritise the fragments that score for it, `--cursor`, `--json`. |
| `graph-expand` | Walk the index's item/topic graph from one item (`--item <id>`): nodes, edges and one explicit path per reached node, each edge with its relation, method, Jaccard weight and supporting item ids — all resolvable in the live store, or the expansion is refused. An edge is co-occurrence **in this corpus**, never a relationship in the world, and the response carries that in its data. Read-only; refuses an index behind the store. `--max-hops` (1), `--max-neighbors` (10), `--json`. See [docs/knowledge-index.md](docs/knowledge-index.md#the-graph--opt-in-and-measured-negative). |
| `mcp-serve` | Serve `xbrain.search`, `xbrain.get` and `xbrain.graph_expand` to an agent over MCP (stdio): the same services and response models as the CLI's `--json`, the CLI's error messages, read-only, retrieved text labelled as untrusted data. Needs the optional `[mcp]` extra. See [docs/mcp.md](docs/mcp.md). |
...
| `generate` | Render the wiki into the vault. Renders a localized **verification badge** (❌ FAIL / ⚠️ REVIEW; a PASS is left unbadged) under a summary / topics / video-digest **only when its stored verdict is still current** — it recomputes the verdict's `contract_fingerprint`, a sha256 over the item's current output, the source the judge read for that target and the rubrics applied, and badges only on a match, silently skipping a **stale** verdict (the output was re-generated, the source changed, or a rubric was rewritten since it was judged; a verdict stored without that fingerprint is always stale). So a FAIL whose output was fixed afterwards never shows a ❌. |
...
# →    · [video_transcript] origin=asr trust=machine_extracted · via lexical
# →      do kind of position dependence and incorporate information from …
# →    · [video_transcript] origin=asr trust=machine_extracted · via lexical
...
lexical one (`uv sync --extra embeddings`, set `[embeddings].command`, then
`uv run xbrain index build --embeddings --force`) and queried with
`--strategy vector` or `--strategy hybrid` (bm25 and embeddings fused by
Reciprocal Rank Fusion, every match saying which channel found it). **It is not
the default, and nothing here recommends switching**: the bake-off that was meant
to choose the model is **incomplete** — it measured 1 of the ≥ 3 candidates its
acceptance criterion requires, and that one did not beat lexical on the strata that
decide ([docs/embeddings-bakeoff.md](docs/embeddings-bakeoff.md)). Without an
embedder, `hybrid` answers lexically and declares why, and `vector` is an error.

**The graph: explore, don't rank.** The index also holds a small graph of items
and topics — an item *has* its topics, and two topics *co-occur* when enough items
carry both (Jaccard-weighted, thresholds measured). `graph-expand` walks it, and
every path carries the ids of the items that support it:

```bash
uv run xbrain graph-expand --item 2063609922667815064 --max-hops 2 --json
```

An edge means "assigned together in this corpus", never "related in the world",
and the response says so in its data. Re-ranking search results by graph
neighbourhood (`hybrid_graph`) was measured and **helps nothing**: across 16
threshold settings it lowered `recall@10` and lifted none of the 33 results only
the graph could reach ([docs/graph-threshold-sweep.md](docs/graph-threshold-sweep.md)).
So it is off, and `--strategy hybrid_graph` answers lexically and says so.

**Give it to an agent.** `xbrain mcp-serve` serves `search`, `get` and
`graph_expand` over MCP (stdio) — the same models as `--json`, read-only, with
retrieved text labelled as untrusted data. It needs the optional `[mcp]` extra:

```bash
claude mcp add xbrain -- uv run --directory /path/to/xbrain --extra mcp xbrain mcp-serve
```

Setup, tool schemas, errors and the trust boundary:
**[docs/mcp.md](docs/mcp.md)** · how an agent should search, verify and cite:
**[docs/knowledge-for-agents.md](docs/knowledge-for-agents.md)**.

What each piece does, costs, staleness, error messages and the rest of the
limits: **[docs/knowledge-index.md](docs/knowledge-index.md)**.
...
| [docs/knowledge-index.md](docs/knowledge-index.md) | Operating the search index: when to rebuild, measured cost, what the lexical baseline cannot do, and the opt-in vector plane with its degradation matrix. |
| [docs/embeddings-bakeoff.md](docs/embeddings-bakeoff.md) | The embeddings bake-off: what was measured, on what, why it is incomplete, and why `hybrid` is not the default. |
