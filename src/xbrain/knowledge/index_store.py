...
def open_for_query(
    ...
    *,
    ...
    THE THREE INPUTS ARE TAKEN AS A UNIT, not two of them with the rest optional (P1a). The
    ported signature let `vocab_path` and `topics_path` default to `None`, and in this tree
    `StoreSignal.of` cannot be called that way — deliberately. An omitted path does not mean
    "do not compare it", it means the input is ABSENT, and two signals filled that way compare
    EQUAL forever however the file moves: the false-negative direction this signal exists never
    to fail in, and the one `StoreSignal.from_dict` refuses a short manifest over. A door that
    could be handed one path of three would reintroduce it one keyword at a time.
...
def _degraded(
    manifest: Manifest, items_path: Path, vocab_path: Path, topics_path: Path
) -> tuple[str, ...]:
