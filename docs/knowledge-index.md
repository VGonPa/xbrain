...
**13 of 15 are met**. §13.1 and §13.5 are not. The table gives each one's state
and where to check it. Test paths are under `tests/`. The count lives in this
sentence, not in the heading, so a change of state does not move the anchor
that other documents link to.
...
| 13.14 | README, tutorial, arquitectura y troubleshooting se actualizan con el código de cada plan | README *Search & retrieval*; ARCHITECTURE *The minimal graph* and *The MCP server*; troubleshooting *The knowledge index*; tutorial §7 *Search by meaning*, *Look around a post: `graph-expand`* and *Hand it to an agent: MCP* | Met since Plan 06.3. The check was a run of the whole tutorial, in order, on a copy of `store-2495`, starting from a `config.toml` copied from `config.toml.example` (2026-09-17). It covered §3's export → fill → `--apply` sequence and then every command in the three sections: `index build --embeddings --force` with `scripts/xbrain-embed` and MiniLM, `search --strategy hybrid`, `graph-expand --max-hops 2`, `claude mcp add` + `claude mcp get` (`✔ Connected`), and one `xbrain.search` call over stdio. Not run: `sync` and the other re-fetches from X, which need a real X session; `digest-video`, whose one candidate was a 487 MB video; and `verify --apply`. `store-2495`'s own LLM outputs stood in for the Claude Code session that fills the worksheets. An earlier check ran only the three sections and missed two breaks in the steps before them. The tutorial caught up after Plans 03 and 04, not with them. |
